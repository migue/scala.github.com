...
**Traducción e interpretación por Miguel Ángel Pastor Olivar**

## Motivación

La metaprogramación en tiempo de compilación ha sido reconocida como una herramienta valiosa que habilita técnicas de programación como:

* Virtualización del lenguage (sobrecargando/sobreescribiendo la semántica del lenguaje original, para permitir una inclusión profunda de lenguajes de dominio específicos (DSLs)) 
* Cosificación de programas (facilitando programas capaces de inspeccionar su propio código),
* Auto optimización(auto aplicación de optimizaciones específicas de un deerminado dominio basadas en la cosificación de programas),
* Construcción algorítmica de programas (generación de código que habitualmente es tedioso de escribir con las abstracciones ofrecidas por un lenguaje de programación).

En este resume se introduce el sistema de macros en Scala. Esta habilitdad permite a los programadores escribir `macro defs`: funciones que son cargadas de manera transparente por el compilador y que son ejecutadas por este último durante el proceso de compilación. De manera resumida esto es la noción de metaprogramación en tiempo de compilación en Scala.

## Intuición

A continuación se muestra una definición tipo de una macro:

    def m(x: T): R = macro implRef

A primera vista las definiciones de macros son equivalentes a las definiciones de funciones tradicionales, excepto por el cuerpo de la función, el cual comienza con la palabra reservada condicional `macro` y es seguida por un identificador posiblemente calificado el cual hace referencia una implemetación estática de la macro.

Si, durante la fase de comprobación de tipos, el compilador encuentra una aplicación de una macro `m(args)`, expandirá dicha
